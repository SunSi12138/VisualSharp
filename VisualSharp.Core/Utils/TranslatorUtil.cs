...
public static class TranslatorUtil
{
    ...
    const string TemporaryVariablePrefix = "temp";
    const int TemporaryVariableNameLength = 16;
    public static readonly Dictionary<MemberVisibility, string> VisibilityTokens = new()
    {
    ...
    static void AddDependentPureNodes(Node node, ref HashSet<Node> nodes)
    {
    ...
    static void AddAllNodes(Node node, ref HashSet<Node> nodes)
    {
        nodes.Add(node);

        foreach (OutputExecPin pin in node.OutputExecPins)
        {
        ...
        foreach (InputDataPin pin in node.InputDataPins)
        {
        ...
        foreach (InputTypePin pin in node.InputTypePins)
        {
            if (pin.IncomingPin != null && !nodes.Contains(pin.IncomingPin.Node))
            {
                AddAllNodes(pin.IncomingPin.Node, ref nodes);
    ...
    static void AddExecNodes(Node node, ref HashSet<Node> nodes)
    {
        nodes.Add(node);
